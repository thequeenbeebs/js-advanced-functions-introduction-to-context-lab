...
function allWagesFor(employeeRecord) {
    let allWages = 0
    employeeRecord.timeInEvents.forEach((timeInObject) => {
        allWages += wagesEarnedOnDate(employeeRecord, timeInObject.date)
    })
    return allWages
}
...
function calculatePayroll(arrayOfEmployees) {
    let payroll = 0
    arrayOfEmployees.forEach(employeeRec => {
        payroll += allWagesFor(employeeRec)
    })
    return payroll
}

function findEmployeeByFirstName(employeeRecords, name) {
    return employeeRecords.find(employeeRec => employeeRec.firstName === name)
